...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const uploadPDF = async (payload: convertPDFModel): Promise<ListOfSOADetails | null> => { // Changed return type to allow null on error
    try {
      ...
      Object.entries(payload).forEach(([key, value]) => {
        if (value !== undefined) { // Only append if value is not undefined
          ...
          } else if (value !== null) { // Also check for null
      ...
      const response: AxiosResponse<ListOfSOADetails> = await api.post('/pdf/upload-pdf', formData, {
        headers: {
    ...
    } catch (err: any) { // Catch block for error handling
